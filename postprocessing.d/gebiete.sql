...
CREATE TEMPORARY TABLE pp_gemarkungen AS
	SELECT
		...
		gemarkungsnummer,
		coalesce(
			(SELECT bezeichnung FROM ax_gemarkung b WHERE a.land=b.land AND a.gemarkungsnummer=b.gemarkungsnummer AND b.endet IS NULL),
			'(Gemarkung '||land||gemarkungsnummer||')'
		) AS gemarkungsname
	FROM ax_flurstueck a
	WHERE endet IS NULL
	GROUP BY land, regierungsbezirk, kreis, gemeinde, gemarkungsnummer
	ORDER BY land, regierungsbezirk, kreis, gemeinde, gemarkungsnummer;

CREATE INDEX pp_gemarkungen_lrkg ON pp_gemarkungen(land, regierungsbezirk, kreis, gemeinde);
CREATE INDEX pp_gemarkungen_lg ON pp_gemarkungen(land, gemarkungsnummer);
ANALYZE pp_gemarkungen;

SELECT alkis_dropobject('ax_flurstueck_lgf');
CREATE INDEX ax_flurstueck_lgf ON ax_flurstueck(land,gemarkungsnummer,flurnummer);

CREATE TEMPORARY TABLE pp_gemeinden AS
	SELECT
		land, regierungsbezirk, kreis, gemeinde,
		coalesce(
			(SELECT bezeichnung FROM ax_gemeinde b WHERE a.land=b.land AND a.regierungsbezirk=b.regierungsbezirk AND a.kreis=b.kreis AND a.gemeinde=b.gemeinde AND b.endet IS NULL),
			'(Gemeinde '||land||regierungsbezirk||kreis||gemeinde||')'
		) AS gemeindename
	FROM pg_temp.pp_gemarkungen a
	GROUP BY land, regierungsbezirk, kreis, gemeinde
	ORDER BY land, regierungsbezirk, kreis, gemeinde;

CREATE INDEX pp_gemeinden_lrkg ON pp_gemeinden(land, regierungsbezirk, kreis, gemeinde);
ANALYZE pp_gemeinden;

CREATE TEMPORARY TABLE pp_kreise AS
	SELECT
		land, regierungsbezirk, kreis,
		coalesce(
			(SELECT bezeichnung FROM ax_kreisregion b WHERE a.land=b.land AND a.regierungsbezirk=b.regierungsbezirk AND a.kreis=b.kreis AND b.endet IS NULL),
			'(Kreis '||land||regierungsbezirk||kreis||')'
		) AS kreisname
	FROM pg_temp.pp_gemeinden a
	GROUP BY land, regierungsbezirk, kreis
	ORDER BY land, regierungsbezirk, kreis;

CREATE INDEX pp_kreise_lrk ON pp_kreise(land, regierungsbezirk, kreis);
ANALYZE pp_kreise;
...
INSERT INTO po_polygons(gml_id,thema,layer,signaturnummer,sn_randlinie,modell,polygon)
	SELECT
		...
		'ax_flurstueck_flur_'||land||gemarkungsnummer||coalesce(flurnummer,0) AS layer,
	...
	FROM ax_flurstueck
	JOIN pg_temp.pp_gemarkungen USING (land,gemarkungsnummer)
	WHERE endet IS NULL
	GROUP BY land, gemarkungsnummer, flurnummer;
...
INSERT INTO po_labels(gml_id,thema,layer,point,text,signaturnummer,drehwinkel,modell)
	SELECT
		...
		'Flur '||replace(layer, 'ax_flurstueck_flur_'||land||gemarkungsnummer, '') AS text,
		'pg-flur' AS signaturnummer,
		0 AS drehwinkel,
		ARRAY['norGIS'] AS modell
	FROM po_polygons
	JOIN pg_temp.pp_gemarkungen p ON layer LIKE 'ax_flurstueck_flur_'||land||gemarkungsnummer||'%' ESCAPE '?';
...
INSERT INTO po_polygons(gml_id,thema,layer,signaturnummer,sn_randlinie,modell,polygon)
	SELECT
		min(gml_id) AS gml_id,
		'Politische Grenzen' AS thema,
		'ax_flurstueck_gemarkung_'||land||gemarkungsnummer AS layer,
	...
	FROM po_polygons
	JOIN pg_temp.pp_gemarkungen p ON layer LIKE 'ax_flurstueck_flur_'||land||gemarkungsnummer||'%' ESCAPE '?'
	GROUP BY land, gemarkungsnummer;
...
INSERT INTO po_labels(gml_id,thema,layer,point,text,signaturnummer,drehwinkel,modell)
	SELECT
		...
		ARRAY['norGIS'] AS modell
	FROM po_polygons
	JOIN pp_gemarkungen p ON layer='ax_flurstueck_gemarkung_'||land||gemarkungsnummer;
...
INSERT INTO po_polygons(gml_id,thema,layer,signaturnummer,sn_randlinie,modell,polygon)
	SELECT
		min(gml_id) AS gml_id,
		'Politische Grenzen' AS thema,
		'ax_flurstueck_gemeinde_'||land||regierungsbezirk||kreis||gemeinde AS layer,
	...
	FROM po_polygons
	JOIN pg_temp.pp_gemarkungen ON layer='ax_flurstueck_gemarkung_'||land||gemarkungsnummer
	GROUP BY land, regierungsbezirk, kreis, gemeinde;
...
INSERT INTO po_labels(gml_id,thema,layer,point,text,signaturnummer,drehwinkel,modell)
	SELECT
		...
		ARRAY['norGIS'] AS modell
	FROM po_polygons
	JOIN pg_temp.pp_gemeinden p ON layer='ax_flurstueck_gemeinde_'||land||regierungsbezirk||kreis||gemeinde;
...
INSERT INTO po_polygons(gml_id,thema,layer,signaturnummer,sn_randlinie,modell,polygon)
	SELECT
		min(gml_id) AS gml_id,
		'Politische Grenzen' AS thema,
		'ax_flurstueck_kreis_'||land||regierungsbezirk||kreis AS layer,
	...
	FROM po_polygons
	JOIN pg_temp.pp_kreise p ON layer LIKE 'ax_flurstueck_gemeinde_'||land||regierungsbezirk||kreis||'%'
	GROUP BY land, regierungsbezirk, kreis;
...
INSERT INTO po_labels(gml_id,thema,layer,point,text,signaturnummer,drehwinkel,modell)
	SELECT
		...
		ARRAY['norGIS'] AS modell
	FROM po_polygons
	JOIN pg_temp.pp_kreise ON layer='ax_flurstueck_kreis_'||land||regierungsbezirk||kreis;
